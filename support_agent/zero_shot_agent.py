...
async def chat():
    ...
    - Initial requests: create new thread and run until interrupt
    - Resume requests: continue with human input using Command(resume=...)
    """
    try:
        ...
        command = data.get('command')  # 'start' or 'resume'
        user_input = data.get('user_input', '')
        
        if command == 'start' or thread_id is None:
            # Create new thread
        ...
        elif command == 'resume' and thread_id:
            # Resume existing thread with user input
            ...
            return jsonify({
                "error": "Invalid command. Use 'start' to begin or 'resume' with thread_id and user_input to continue."
            }), 400
